...
function renderRechnungenLog() {
  ...
  const AKTION = {
    'erstellt':  { label:'Erstellt',           color:'var(--green)',  matIcon:'add',          bg:'rgba(58,138,78,.08)' },
    'geaendert': { label:'Geändert',            color:'var(--blue)',   matIcon:'edit',           bg:'rgba(26,69,120,.08)' },
    'geloescht': { label:'Gelöscht',            color:'var(--red)',    matIcon:'delete',         bg:'rgba(214,51,65,.08)' },
    'bezahlt':   { label:'Bezahlt + Einnahme',  color:'var(--green)',  matIcon:'check_circle',  bg:'rgba(58,138,78,.08)' },
    'status':    { label:'Status geändert',     color:'var(--yellow)', matIcon:'swap_horiz',  bg:'rgba(196,122,18,.08)' },
  };

  if (!log.length) {
    tb.innerHTML = '';
    em.style.display = 'block';
    em.innerHTML = '<div class="ei"><span class="material-symbols-outlined">description</span></div><p>Keine Rechnungs-Änderungen</p>';
    if(pg) pg.innerHTML = '';
    return;
  }
  em.style.display = 'none';
  ...
  pageGroups.forEach(([nr, events]) => {
    ...
    events.forEach((e, idx) => {
      const a = AKTION[e.aktion] || { label:e.aktion, color:'var(--sub)', matIcon:'circle', bg:'var(--s2)' };
        ...
        <div style="display:flex;flex-direction:column;padding:12px 14px;${rowBg}${rowOpacity}">
          <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:${changeHtml?'8':'0'}px">
            <div style="flex:0 0 auto;width:32px;height:32px;border-radius:var(--r);background:${a.bg};display:flex;align-items:center;justify-content:center;margin-top:2px">
              <span class="material-symbols-outlined" style="color:${a.color};font-size:14px">${a.matIcon||'circle'}</span>
            </div>
            <div style="flex:1;min-width:0">
              <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:3px">
