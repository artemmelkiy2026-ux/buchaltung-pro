...
function stBerechnen() {
  ...
  if (!isKleinunternehmer && ein < 25000 && ustSaldo > 100) {
    document.getElementById('st-klein-recommendation').textContent = 
      `<span style="color:var(--green)">${ic('check-circle')}</span> EMPFOHLEN: Sie zahlen ${fmt(ustSaldo)} USt. Als Kleinunternehmer sparen Sie diese Summe!`;
    ...
    document.getElementById('st-klein-recommendation').textContent = 
      `<span style="color:var(--green)">${ic('check-circle')}</span> Kleinunternehmer aktiv — Sie zahlen KEINE Umsatzsteuer!`;
