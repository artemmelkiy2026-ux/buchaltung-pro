...
function showMobDetail(entry){
  ...
  document.getElementById('mdm-title').innerHTML=`<span class="badge ${isEin?'b-ein':'b-aus'}">${isEin?'<span style="color:var(--green)">${ic('arrow-up')}</span> Einnahme':'<span style="color:var(--red)">${ic('arrow-down')}</span> Ausgabe'}</span>`;
  ...
  document.getElementById('mdm-btns').innerHTML=`
    <button class="btn primary" style="flex:1" onclick="closeMobDetail();editE(event,'${entry.id}')">${ic('edit')} Bearbeiten</button>
    ...
    return `<div class="kunde-card" onclick="showKundeRechnungen('${k.id}')">
      ...
      <div class="kunde-card-body">
        ...
        <div class="kunde-card-meta">
          ${k.email?`<a href="mailto:${k.email}" onclick="event.stopPropagation()" class="kunde-meta-link">${ic('envelope')} ${k.email}</a>`:''}
          ${k.tel?`<span class="kunde-meta-item">${ic('phone')} ${k.tel}</span>`:''}
          ${k.ort?`<span class="kunde-meta-item">${ic('map-marker-alt')} ${k.plz?k.plz+' ':''}${k.ort}</span>`:''}
        </div>
      </div>
      <div class="kunde-card-right">
        ...
        <div class="kunde-card-actions" onclick="event.stopPropagation()">
          <button class="rca-btn" onclick="editKunde('${k.id}')" title="Bearbeiten">${ic('edit')}</button>
          <button class="rca-btn" onclick="neueRechnungFuerKunde('${k.id}')" title="Rechnungen">${ic('file-invoice')}</button>
          <button class="rca-btn rca-red" onclick="delKunde('${k.id}')" title="Löschen">${ic('trash')}</button>
        </div>
      </div>
    </div>`;
  }).join('');
...
function saveUstMode(){
  ...
  _ustSaving = false;
  toast(sel.value==='§19'
    ? `<span style="color:var(--green)">${ic('check-circle')}</span> ${yr}: §19 Kleinunternehmer gesetzt`
    : `${ic('chart-bar')} ${yr}: MwSt gesetzt`, 'ok');
}
  ...
  if(!filteredMwst.length){
    if(empty2){
      ...
      empty2.innerHTML = ustQuartalFilter > 0
        ? `<div class="ei">${ic('calendar-times')}</div><p>Keine Buchungen in Q${ustQuartalFilter} ${yr}</p><p style="font-size:12px;color:var(--sub);margin-top:4px">Für dieses Quartal liegen keine USt-Buchungen vor</p>`
        : `<div class="ei">${ic('receipt')}</div><p>Keine USt-Buchungen für ${yr}</p>`;
    }
      ...
      pageItems.forEach(e=>{
        ...
        html+=`<div class="ust-row">
          ...
          <div class="ust-row-right">
            ...
            ${canDel
              ? `<button class="rca-btn rca-red" onclick="delUstEintrag('${e.id}')" title="Löschen">${ic('trash')}</button>`
              : `<span class="ust-src-lock" title="Aus ${e.quelle} — dort löschen">${ic('lock')}</span>`
            }
          </div>
        </div>`;
      });
