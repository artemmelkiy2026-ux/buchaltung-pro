  ...
  if(!entries.length){
    ...
    document.getElementById('e-tbody').innerHTML = pageEntries.map(e => {
      ...
      if(e.is_storno&&e.storno_of){
        ...
        stLbl='<span style="font-size:9px;font-weight:700;color:#1c9e4c;background:rgb(34 197 94 / 9%);padding:2px 5px;border-radius:3px;margin-right:4px">✎ Korrektur</span>';
      }
      ...
      return '<div style="display:flex;align-items:center;gap:12px;padding:11px 14px;background:#fff;border:1px solid var(--border);border-radius:12px;margin-bottom:8px;transition:box-shadow .15s,background .15s;cursor:pointer;'+(st?'opacity:0.45;':'')+'"'
        ...
        +mwstBadge
        +'</div></div>'
        +'<div class="prise-1" style="flex:0 0 auto;display:flex;flex-direction:column;align-items:flex-end;gap:4px">'
