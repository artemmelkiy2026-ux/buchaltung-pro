...
const ZICONS={'Überweisung':'<span class="material-symbols-outlined">account_balance</span>','Barzahlung':'<span class="material-symbols-outlined">euro_symbol</span>','PayPal':'<span class="material-symbols-outlined">account_balance_wallet</span>','EC-Karte':'<span class="material-symbols-outlined">credit_card</span>','Lastschrift':'<span class="material-symbols-outlined">swap_horiz</span>','Sonstiges':'<span class="material-symbols-outlined">euro_symbol</span>'};
...
// <span class="material-symbols-outlined" style="color:var(--green)">check_circle</span> Переменные для сортировки Dashboard "Letzte 10"
let dashSortCol='datum', dashSortAsc=false;
// <span class="material-symbols-outlined" style="color:var(--green)">check_circle</span> Переменные для пагинации Einträge
...
function appInit(){
  ...
  setTimeout(()=>{
    ...
    if(due.length) toast(`<span class="material-symbols-outlined">sync</span> ${due.length} ${'wiederkehrende Zahlung'}${due.length>1?'en':''} ${'fällig!'}`, 'ok');
  },800);
}
